...
interface PharmacyCardProps extends Pharmacy {
  badgeLabel: string
  onClick?: () => void
}
    ...
    <section className={cn("flex flex-col gap-6", className)}>
      <header className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-semibold text-[#29196E]">Pharmacy</h1>
          </div>

          <Button
            type="button"
            onClick={() => router.push("/dashboard/pharmacy/create")}
      ...
      {filteredPharmacies.length === 0 ? (
        <div className="rounded-xl border border-dashed border-[#DADBF7] bg-white py-12 text-center text-sm text-[#7A819F]">
          No pharmacies match the provided name.
        </div>
      ) : (
        <div className="grid gap-3 sm:grid-cols-2">
          {filteredPharmacies.map((pharmacy) => (
            <PharmacyCard
              ...
              onClick={() => router.push(`/dashboard/pharmacy/${pharmacy.id}`)}
              {...pharmacy}
            />
          ))}
        </div>
